...
export namespace fae
{
	struct resource_manager
	{
		...
		[[nodiscard]] inline constexpr auto insert_and_get(t_resource &&resource = {}) noexcept -> t_resource &
		{
			...
			m_resources.insert(
				std::make_pair(key, std::forward<t_resource &&>(resource)));
		...
		[[maybe_unused]] inline constexpr auto insert(t_resource &&resource = {}) noexcept -> resource_manager &
		{
			[[maybe_unused]] const auto &res =
				insert_and_get(std::forward<t_resource &&>(resource));
			return *this;
		}
		...
		emplace_and_get(t_args &&...args) noexcept -> t_resource &
		{
			...
			else
			{
				m_resources.insert(std::make_pair(
					key,
					std::move(t_resource{std::forward<t_args &&>(args)...})));
			}
		...
		{
			[[maybe_unused]] const auto &res =
				emplace_and_get<const t_resource>(
		...
		template <typename t_resource>
		[[maybe_unused]] inline constexpr auto
		use_resource(std::function<void(t_resource &)> callback) noexcept
			-> resource_manager &
		{
			const auto key = std::type_index(typeid(t_resource));
			if (m_resources.contains(key))
